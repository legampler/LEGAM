document.addEventListener('DOMContentLoaded', () => {
    ...
    shareBtn.addEventListener('click', () => {
        // Copy link halaman ini (atau link hosting nantinya)
        navigator.clipboard.writeText(window.location.href).then(() => {
            // Tampilkan notifikasi (Toast)
            const toast = document.createElement('div');
            toast.className = 'toast show';
            toast.innerText = 'Link profil berhasil disalin! 📋';
            document.body.appendChild(toast);
            
            // Hapus notifikasi setelah 3 detik
            setTimeout(() => {
                toast.classList.remove('show');
                setTimeout(() => toast.remove(), 400); // Tunggu animasi css selesai
            }, 3000);
        }).catch(err => {
            console.error('Gagal menyalin link: ', err);
            alert('Gagal menyalin link. Silakan copy manual URL di browser.');
        });
    });
});
